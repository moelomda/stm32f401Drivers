...
#define INI_PWR_ON_STATE             0
#define INI_FUN_SET_STATE            1
#define INI_DIS_ONOFF_CLR_STATE      2
#define INI_DIS_CLR_STATE            3
#define INI_END_STATE                4
...
{
    static u8 InitState = INI_PWR_ON_STATE ;
    static u8 Counter = 0 ;
    Counter++;
    switch (InitState)
    {
    case INI_PWR_ON_STATE :
    	if(Counter > 32)
    	{
    		LCD_HelperWriteCommand(LCD_8BIT_MODE);
    		if(LCD_LatchedEn){
    			InitState=INI_FUN_SET_STATE;
    		    Counter=0;
    		}
    	}
    break;
    case INI_FUN_SET_STATE :
        if(Counter == 1)
        {
           LCD_HelperWriteCommand(LCD_DISPLAY_CURSOR_BLINK_ALL_ON);
       	if(LCD_LatchedEn){
       	   InitState=INI_DIS_ONOFF_CLR_STATE;
       	   Counter=0;
       	  }
        }
    break;
    case INI_DIS_ONOFF_CLR_STATE :
    	  if(Counter == 1)
    	  {
    	   LCD_HelperWriteCommand(LCD_CLEAR_DISPLAY);
    	   if(LCD_LatchedEn){
    		 InitState=INI_DIS_CLR_STATE;
    	     Counter=0;
    	    }
    	  }
    break;
    case INI_DIS_CLR_STATE :
    	 if(Counter == 2)
    	 {
    	   LCD_HelperWriteCommand(LCD_CLEAR_DISPLAY);
    	   if(LCD_LatchedEn){
        	  Counter=0;
        	  InitState=INI_END_STATE;
        	}
    	 }
    break;
    case INI_END_STATE :
